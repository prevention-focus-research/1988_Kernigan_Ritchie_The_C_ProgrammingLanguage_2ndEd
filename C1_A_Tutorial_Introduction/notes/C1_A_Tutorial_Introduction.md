...
`main` and `getline` communicate through a pair of arguments and a returned value. In `getline`, the arguments are declared by the line `int getline(char s[], int lim)` which specifies that the first argument, `s`, ...
